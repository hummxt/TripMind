...
export default function FoodFinderPage() {
    ...
    const generateFoodPlan = async (formData: any) => {
        setIsGenerating(true);
        setError(null);
        window.scrollTo({ top: 0, behavior: "smooth" });

        try {
            const res = await fetch("/api/food-discovery", {
                method: "POST",
            ...
            if (!res.ok) {
                throw new Error(data.error || "Failed to analyze food options");
            }
    ...
    return (
        <div className="min-h-screen flex flex-col relative">
            ...
            <main className="flex-grow max-w-7xl mx-auto w-full pt-32 pb-20 px-6 flex flex-col items-center relative z-10">
                <div className={`absolute top-0 left-0 w-[50%] h-[50%] bg-warning/5 blur-[200px] pointer-events-none`} />

                <div className="w-full max-w-6xl relative text-center">
                    {!foodData && !isGenerating && (
                        <div className="space-y-16 animate-in fade-in slide-in-from-bottom-10 duration-700">
                            ...
                            <div className="flex justify-center">
                                <FoodFinderForm onGenerate={generateFoodPlan} />
                            </div>
                        </div>
                    )}
                    ...
                    {isGenerating && (
                        <div className="flex flex-col items-center gap-10 py-32 animate-in zoom-in-95 duration-500">
                            <div className="relative">
                                <div className="w-32 h-32 border-[6px] border-warning/10 border-t-warning rounded-full animate-spin" />
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <span className="text-4xl">🍳</span>
                                </div>
                            </div>
                            <div className="space-y-4">
                                <h3 className="text-5xl font-black tracking-tighter text-warning">{t("analyzing")}</h3>
                        ...
                        <div className="animate-in zoom-in-95 duration-700 w-full flex flex-col items-center">
                            <div className="mb-12 space-y-3 text-center">
                                ...
                                <h2 className="text-4xl md:text-5xl font-black tracking-[-0.03em]">
                                    {language === "en" ? "Culinary Analysis Ready" : language === "az" ? "Qida Analizi Hazırdır" : "Кулинарный анализ готов"}
                                </h2>
                                <p className="text-white/50 dark:text-neutral-500 font-bold tracking-widest uppercase text-xs">Curated by TripMind Gastronomy Agent AI</p>
                            </div>
                            <FoodDisplay data={foodData} />
